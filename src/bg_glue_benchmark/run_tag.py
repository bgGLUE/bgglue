...
class DataTrainingArguments:
    ...
    def __post_init__(self):
        if self.dataset_name is None and self.data_dir is None:
            raise ValueError("Need either a dataset name or a data dir.")
...
def main():
    ...
    if training_args.do_predict:
        ...
        # Save predictions
        output_predictions_file = os.path.join(training_args.output_dir, "predictions.jsonl")
        if trainer.is_world_process_zero():
            with open(output_predictions_file, "w") as writer:
                for index, prediction in enumerate(true_predictions):
                    json_line = {"index": index, "label": " ".join(prediction)}

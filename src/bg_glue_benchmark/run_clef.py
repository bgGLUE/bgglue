...
""" Finetuning multi-lingual models on CLEF (e.g. Bert, DistilBERT, XLM).
...
def main():
    ...
    # Downloading and loading clef dataset from the hub.
    ...
    metric = load_metric(str(pathlib.Path(__file__).parent.resolve() / "metrics_clef.py"))
    ...
    if training_args.do_predict:
        ...
        if trainer.is_world_process_zero():
            with open(output_predict_file, "w") as writer:
                for index, (item, example) in enumerate(zip(predictions, predict_dataset)):
                    # The format is compatible with the official evaluation script from the competition
                    json_line = {
                        "topic": example["topic_id"],
                        "id": str(example["id_str"]),
                        "label": float(item[1]),
